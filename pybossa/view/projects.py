...
@blueprint.route('/categorias/destacados/', defaults={'page': 1})
@blueprint.route('/categorias/destacados/page/<int:page>/')
def index(page):
...
@blueprint.route('/categorias/borradores/', defaults={'page': 1})
@blueprint.route('/categorias/borradores/pagina/<int:page>/')
@login_required
@admin_required
def draft(page):
...
@blueprint.route('/categorias/<string:category>/', defaults={'page': 1})
@blueprint.route('/categorias/<string:category>/page/<int:page>/')
...
@blueprint.route('/<short_name>/tasks/taskpresentereditorpreview', methods=['GET', 'POST'])
@login_required
def taskpresentereditorpreview(short_name):
    errors = False
...
def import_task(short_name):
    ...
    loading_text = gettext("Importando tareas, espere...")
    pro = pro_features()
...
def _import_tasks(project, **form_data):
    ...
    if number_of_tasks <= MAX_NUM_SYNCHRONOUS_TASKS_IMPORT:
        ...
        flash(gettext(u"Está intentando importar una gran cantidad de tareas, así que tenga paciencia.Recibirás un correo electrónico cuando las tareas estén listas."))
    return redirect_content_type(url_for('.tasks',
                                         short_name=project.short_name))
...
@blueprint.route('/<short_name>/tarea/<int:task_id>')
...
@blueprint.route('/<short_name>/contribuir')
...
@blueprint.route('/<short_name>/tareas/')
...
@blueprint.route('/<short_name>/tareas/browse')
@blueprint.route('/<short_name>/tareas/browse/<int:page>')
...
def delete_tasks(short_name):
    ...
    if request.method == 'GET':
        ...
        msg = gettext(u"Se han eliminado las tareas y las colaboraciones asociadas.")
        flash(msg, 'success')
...
@blueprint.route('/<short_name>/estadisticas')
...
def task_n_answers(short_name):
    ...
    if request.method == 'GET':
        ...
        msg = gettext(u'¡Redundancia de tareas actualizada!')
        flash(msg, 'message')
        return redirect_content_type(url_for('.tasks', short_name=project.short_name))
    else:
        flash(gettext('Por favor corrige los errores'), 'error')
        response = dict(template='/projects/task_n_answers.html',
                        title=title,
                        form=form,
                        project=project_sanitized,
...
def publish(short_name):
    ...
    flash(gettext(u'Proyecto publicado! ¡Los voluntarios ahora podrán ayudarte!'))
    return redirect(url_for('.details', short_name=project.short_name))

@blueprint.route('/<short_name>/unpublish', methods=['GET', 'POST'])
@login_required
def unpublish(short_name):
    ...
    ensure_authorized_to('publish', project)
    if request.method == 'GET':
        return render_template('projects/unpublish.html',
                                project=project,
                                pro_features=pro)
    ...
    result_repo.delete_results_from_project(project)
    webhook_repo.delete_entries_from_project(project)
    auditlogger.log_event(project, current_user, 'update', 'unpublished', False, True)
    flash(gettext(u'Proyecto despublicado correctamente.'))
    return redirect(url_for('.details', short_name=project.short_name))
