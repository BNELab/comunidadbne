...
def index():
    ...
    users = dict(label="User Statistics",
                 values=[
                     dict(label=u'Anónimos', value=[0, n_anon]),
                     dict(label='Registrados', value=[0, n_auth])])

    projects = dict(label="Projects Statistics",
                    values=[
                        dict(label='Publicados',
                             value=[0, n_published_projects]),
                        dict(label='Borradores', value=[0, n_draft_projects])])

    tasks = dict(label="Task and Task Run Statistics",
                 values=[
                     dict(label='Tareas', value=[0, n_tasks]),
                     dict(label='Respuestas', value=[1, n_task_runs])])
