...
def get_candidate_task_ids(project_id, user_id=None, user_ip=None,
    ...
    valoresvalidos=True
    idsomitidos = request.cookies.get("omitir"+str(project_id))

    if idsomitidos is None:
        omitidas = []
    else: 
        omitidas = idsomitidos.split("%3B")

        for domitidas in omitidas:
            esentero = RepresentsInt(domitidas)
            if esentero == False:
    ...
    if user_id and not user_ip and not external_uid:
        ...
        #    subquery = session.query(TaskRun.task_id).filter_by(project_id=project_id, user_id=user_id)
        #else: 
        #    subquery = session.query(Task.id).filter_by(project_id=project_id).outerjoin(TaskRun).filter(or_(TaskRun.user_id==user_id,Task.id.in_(omitidas)))
        ...
        if user_ip and not external_uid:
            ...
            #    subquery = session.query(TaskRun.task_id).filter_by(project_id=project_id, user_ip=user_ip)
            #else:
            #    subquery = session.query(Task.id).filter_by(project_id=project_id).outerjoin(TaskRun).filter(or_(TaskRun.user_ip==user_ip,Task.id.in_(omitidas)))
            ...
            #    subquery = session.query(TaskRun.task_id).filter_by(project_id=project_id, external_uid=external_uid)
            #else:
            #    subquery = session.query(Task.id).filter_by(project_id=project_id).outerjoin(TaskRun).filter(or_(TaskRun.external_uid==external_uid,Task.id.in_(omitidas)))
...
def sched_variants():
    return [('default', 'Por defecto'), ('breadth_first', 'Amplitud primero'),
            ('depth_first', 'Profundidad primero')]
