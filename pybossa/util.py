...
def pretty_date(time=False):
    ...
    if day_diff == 0:
        if second_diff < 10:
            return "ahora"
        if second_diff < 60:
            return str(second_diff) + " segundos"
        if second_diff < 120:
            return "un minuto"
        if second_diff < 3600:
            return ' '.join([str(second_diff / 60), "minutos"])
        if second_diff < 7200:
            return "una hora"
        if second_diff < 86400:
            return ' '.join([str(second_diff / 3600), "horas"])
    if day_diff == 1:
        return "ayer"
    if day_diff < 7:
        msgd = u'días'; return ' '.join([str(day_diff),msgd])
    if day_diff <= 13:
       return 'una semana'
    if day_diff < 31:
        return ' '.join([str(day_diff / 7), "semanas"])
    if day_diff < 60:
        return ' '.join([str(day_diff / 30), "mes"])
    if day_diff < 365:
        return ' '.join([str(day_diff / 30), "meses"])
    if day_diff < (365 * 2):
        msga = u'año'; return ' '.join([str(day_diff / 365), msga])
    msgas = u'años'; return ' '.join([str(day_diff / 365), msgas])
